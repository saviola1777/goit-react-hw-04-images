  ...
  const Modal =({largeImg, close })=>{                          // передаємо пропсами велику картинку і функцію яка закриває модалку міняє на false
    ...
    const closeModalClick =({ target, currentTarget }) => {   // деструктупизація на що саме ми клікнули code це назва клавіши
      if (target === currentTarget ) {      //якшо клікнемо поза картинкою або нажмемо на ускейп виклече функцію яка закриває модалку
        close()
      }
    }
    
    useEffect(()=>{   
      const closeModal =({code }) => {   // деструктупизація на що саме ми клікнули code це назва клавіши
        if (code === "Escape") {      //якшо клікнемо поза картинкою або нажмемо на ускейп виклече функцію яка закриває модалку
          close()
        }
      }  
